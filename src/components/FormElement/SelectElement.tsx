...
export default function SelectElement({ label, value, onChange ,option=[]}: any) {
  return (
    <div className="relative">
      <p className="pt-0 pr-2 pb-0 pl-2 -mt-3 mr-0 ml-2 font-medium text-gray-600">
        {label}
      </p>
      <select
        value={value}
      ...
      >
        {
            option && option.length?option.map(({item}:any)=><option id={item.id} value={item.id} key={item.id}></option> ):<option id="" value="">Select</option>
        }
      </select>
    </div>
  );
